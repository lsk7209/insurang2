-- 안전한 마이그레이션: 기존 테이블에 컬럼 추가
-- SQLite는 ALTER TABLE ADD COLUMN만 지원하므로, 각 컬럼을 개별적으로 추가

-- offers 테이블에 컬럼 추가 (이미 존재하면 에러 발생하므로 주의)
-- 각 컬럼을 개별적으로 추가하여, 이미 존재하는 경우 에러를 무시할 수 있도록 함

-- title 컬럼 추가 (없는 경우에만)
-- SQLite는 컬럼 존재 여부를 확인할 수 없으므로, 에러가 발생해도 무시하고 진행

-- thumbnail 컬럼 추가
-- status 컬럼 추가
-- download_link 컬럼 추가
-- json_ld 컬럼 추가
-- ab_test_variant 컬럼 추가

-- 나머지 테이블들은 CREATE TABLE IF NOT EXISTS로 안전하게 생성
